      ...
      <View style={styles.container}>
        ...
        <TouchableOpacity>
          <View style={styles.ViewBackChat}>
            <Ionicons style={{ color: "#6B50F6" }} name="chevron-back-outline" size={24} color="black" />
          </View>
        </TouchableOpacity>
        <Text style={styles.TitleChat}>
          Chat
        </Text>
        {/* khung body tin nhắn */}
        {messages.map((message, index) => (
          <View style={styles.chating} key={index}>
            <View style={{ ...styles.photoProfile, backgroundColor: message.backgroundColor }}>
              <Image
            ...
            <View style={styles.NameAndNotification}>
              <Text style={{ marginLeft: 10, marginTop: 7 }}>{message.name}</Text>
              <Text style={{ marginLeft: 10, marginTop: 7, opacity: 0.3 }}>{message.message}</Text>
            </View>
